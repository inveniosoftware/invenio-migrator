...
def import_record(data, source_type=None, latest_only=False):
    ...
    recorddump = current_migrator.records_cls(
        data, source_type, latest_only,
        pid_fetchers=current_migrator.records_pid_fetchers)
    recorddump.prepare_data()

    try:
        recorddump.import_data()
    except Exception:
        db.session.rollback()
        raise
    ...
    if current_migrator.records_post_task:
        current_migrator.records_post_task.delay(
            recorddump.recid, recorddump.record_uuid
        )
