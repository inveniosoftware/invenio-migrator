...
from .tasks import import_record
...
def loadrecords(source, source_type):
    ...
    with click.progressbar(data) as records:
        for item in records:
            import_record.delay(data, source_type=source_type)
...
def inspectrecords(source, recid, entity=None):
    ...
    data = filter(lambda d: d['recid'] == recid, data)
    ...
    for record in data:
        ...
        if entity == 'json':
            ...
            for revision in record['json']:
                click.secho('Revision', fg='yellow')
                click.echo(json.dumps(revision, indent=2))

        if entity == 'marcxml':
            click.secho('Records (MARCXML)', fg='green')
            for revision in record['marcxml']:
                click.secho('Revision', fg='yellow')
                click.echo(revision)
